...
\usepackage{float}
\usepackage{graphicx}
\usepackage{amsmath, amsfonts, amssymb,esint,mathtools}
\usepackage{trfsigns}
\usepackage{bm}
\usepackage{empheq}
\usepackage{siunitx}
...
\subsection{Berechnung der Transversal- aus den Longitudinalkomponenten}
	...
	\end{align*}
\subsection{Helmholtzgleichungen}
	\begin{equation*}
		\Delta\vc{H}+\beta^2_\text{c}\vc{H}=0 \qquad \Delta\vc{E}+\beta^2_\text{c}\vc{E}=0
	\end{equation*}
\HL
...
\pagebreak
\section*{Wellen und Wellenleiter}
\subsection*{Hohlleiter}
Übertragung von TE- (H-) und TM- (E-) Moden; TEM-Übetragung nicht möglich;\\erwünscht ist i.d.R. auschließlich der $\text{H}_{10}$-Mode
	\begin{equation*}
	...
	\begin{equation*}
		f_{\text{c},m,n} = \frac{c}{2}\cdot\sqrt{\left(\frac{m}{a}\right)^2+\left(\frac{n}{b}\right)^2}	\qquad f_\text{c}=\frac{\beta_\text{c}}{2\pi\sqrt{\varepsilon\mu}}=\frac{f_{\text{c}0}}{\sqrt{\varepsilon_r\mu_r}} \qquad \beta^2_\text{c}=\beta^2_0-\beta^2 \qquad \beta_0=\omega\sqrt{\varepsilon\mu}
	\end{equation*}
	Flächenstromdichte auf dem Rand (Auch für Bandleitung geeignet)
	\begin{equation*}
		\vc{J}_\text{F}(x=0,y,z)=\vec{u}_\text{x}\times\vc{H}(x=0,y,z)
	\end{equation*}
\subsection*{Zweileitersysteme allgemein}
Übertragung von TEM-Wellen mit der Grenzfrequenz $f_c=0$
...
\subsection{Koaxialleitung}
	Wellenwiderstand, Leistung und Leitungsbeläge
	\begin{equation*}
		C' = \frac{C}{l} = \frac{2\pi\varepsilon}{\ln\left(\frac{r_\text{a}}{r_\text{i}}\right)} \qquad L' = \frac{\varepsilon\mu}{C'} = \frac{\mu\ln\left(\frac{r_\text{a}}{r_\text{i}}\right)}{2\pi}
	\end{equation*}
	\begin{equation*}
		Z_\text{L}=\frac{1}{2\pi}Z_{\text{F}0}\ln\left(\frac{r_\text{a}}{r_\text{i}}\right) \qquad P=\frac{1}{2}Z_\text{L}I^2_{\text{max}}=\frac{1}{2}\frac{U^2_{\text{max}}}{Z_\text{L}}=\frac{1}{2}\int^{2\pi}_0 \int^{r_\text{a}}_{r_\text{i}} \frac{E(r)^2}{Z_\text{F}}\D A
	\end{equation*}
...
\subsection{Bandleitung}
	\begin{equation*}
		\text{TEM: }\vc{E}=-U_0\frac{1}{b}e^{\mp\j\beta z}\vec{u}_\text{y} \qquad \text{TE: }\vc{H}_\text{z}=H_0\cos\left(\frac{n\pi y}{b}\right)e^{\mp\j\beta z} \qquad \text{TM: }\vc{E}_\text{z}=E_0\sin\left(\frac{n\pi y}{b}\right)e^{\mp\j\beta z}
	\end{equation*}
	\begin{equation*}
		\phi =\frac{y}{b} \qquad \vc{E}=-U_0\nabla\phi e^{-\j\beta z}=-\frac{U_0}{b}e^{\mp\j\beta z}\vec{u}_y \qquad \vc{H}=\pm\frac{U_0}{Z_\text{F}b}e^{\mp\j\beta z}\vec{u}_x
	\end{equation*}
	\begin{equation*}
		\beta_\text{c}=\frac{n\pi}{b} \qquad C'=\frac{C}{l}=\frac{\varepsilon a}{b} \qquad L'=\frac{\mu b}{a} \qquad Z_\text{L}=\sqrt{\frac{L'}{C'}}=\sqrt{\frac{\mu}{\varepsilon}}\frac{b}{a}=Z_\text{F}\frac{b}{a}
	\end{equation*}
...
TEM-Wellen
	\begin{equation*}
		\frac{\vc{E}_\text{x}}{\vc{H}_\text{y}} = -\frac{\vc{E}_\text{y}}{\vc{H}_\text{x}} = \pm\frac{\omega\mu}{\beta} = \pm\sqrt{\frac{\mu}{\varepsilon}} = \pm Z_F \qquad Z_{\text{F}0} = \sqrt{\frac{\mu_0}{\varepsilon_0}} = 120\pi\Omega = 377\Omega \qquad v_\text{p}=v_\text{g}=c
	\end{equation*}
\HL
\section{Antennentheorie}
\subsection{Feldgrößen in der Fernfeldnäherung}
Vektorpotential
	\begin{equation*}
		\vc{A}(\vec{r})=\frac{\mu e^{-\j\beta r}}{4\pi r}\iiint_V\vc{J}e^{\j\beta r' \cos(\xi)}\D V'
	\end{equation*}
Feldstärken
	\begin{equation*}
		\vc{B}\approx-\j\beta\vec{u}_r\times\vc{A} \qquad \vc{H}\approx-\frac{\j\beta}{\mu}\vec{u}_r\times\vc{A} \qquad \vc{E}\approx\j\omega\vec{u}_r\times(\vec{u}_r\times\vc{A})
	\end{equation*}
...
Richtfaktor
	\begin{equation*}
		D=\frac{S_{\text{max}}}{S_0}	=\frac{S_{\text{max}}}{P}\cdot4\pi r^2 \qquad \text{bzw.} \qquad D(\vartheta,\varphi)=\frac{S(\vartheta,\varphi)}{S_0} \qquad \boxed{D_\text{Kugel}=\frac{S_0}{S_0}=1}	
	\end{equation*}
...
\subsection{Abstandsabhängige komplexe Wellenamplituden}
	\begin{minipage}{0.55\linewidth}
		\includegraphics[width=\linewidth]{img/wellenleiter}
	\end{minipage}
	\begin{minipage}{0.4\linewidth}
		\begin{align*}
			\c{a}(z)&=\c{a}_0(z)e^{-\j\beta z}\\
			\c{b}(z)&=\c{b}_0(z)e^{+\j\beta z}
		\end{align*}
	\end{minipage}
	\begin{equation*}
		\c{\Gamma}_0=\frac{\c{b}_0}{\c{a}_0}=\frac{\c{Z}-Z_\text{L}}{\c{Z}+Z_\text{L}}=\c{\Gamma}(z=0) \qquad
		\c{\Gamma}(z)=\frac{\c{b}(z)}{\c{a}(z)}=\frac{\c{b}_0e^{+\j\beta z}}{\c{a}_0e^{-\j\beta z}}=\c{\Gamma}_0e^{+\j2\beta z}
	\end{equation*}
	Wenn $z=0$ am Anfang des Leiters muss die Phase von $\c{\Gamma}_0$ zurückgedreht werden. Es gilt (Bsp.):
	\begin{equation*}
		\c{\Gamma}_0=\frac{\c{Z}-Z_\text{L}}{\c{Z}+Z_\text{L}}e^{-\j2\beta l}
	\end{equation*}
	...
	\begin{equation*}
		\c{a}(z)=\frac{1}{2}\left(\frac{\c{U}(z)}{\sqrt{Z_\text{L}}}+ \sqrt{Z_\text{L}}\c{I}(z)\right) \qquad \c{b}(z)=\frac{1}{2}\left(\frac{\c{U}(z)}{\sqrt{Z_\text{L}}}- \sqrt{Z_\text{L}}\c{I}(z)\right) \qquad \c{c}(z)=\c{a}(z)+\c{b}(z)
	\end{equation*}
\HL
\section{Zweitore}
Für Anwendung auf Mehrtore nicht betrachtete Tore reflexionsfrei abschließen
...
Streumatrix
	\begin{equation*}
		\c{\matr{b}} = \c{\matr{S}}\cdot\c{\matr{a}} \qquad \c{\matr{S}}^{\ast\text{T}}\cdot\c{\matr{S}}=\matr{E} \qquad \text{Unitaritätskriterium, Vorraussetzung für Verlustfreiheit}
	\end{equation*}
Transmissionsmatrix
	\begin{align*}
		\begin{pmatrix}
...
Verkettung
	\begin{equation*}
		\c{\matr{T}}=\c{\matr{T}}^{(1)}\cdot\c{\matr{T}}^{(2)} \qquad \boxed{\c{\matr{S}}\neq\c{\matr{S}}^{(1)}\cdot\c{\matr{S}}^{(2)}\textbf{!!!}}
	\end{equation*}
\subsection{Streuparameter}
	\includegraphics[width=0.5\linewidth]{img/streuparameter}
	\mbox{(Oft statt $Z_\text{L}$ auch $R_\text{N}$)}

Reflexions- und Transmissionsfaktoren
	\begin{equation*}
		\c{S}_{ii}=\frac{\c{b}_i}{\c{a}_i}=\frac{\c{Z}_{\text{E}i}-Z_{\text{L}i}}{\c{Z}_{\text{E}i}+Z_{\text{L}i}} \qquad \c{S}_{ji}=\frac{\c{b}_j}{\c{a}_i}=2\frac{\c{U}_j}{\c{U}_{0i}}\sqrt{\frac{Z_{\text{L}i}}{Z_{\text{L}j}}} \qquad \parbox{0.4\linewidth}{Statt Einzelspannungen Spannungsteiler einsetzen. Auf Parallelschaltungen achten}
	\end{equation*}
unterschiedliche Leitungslängen
	\begin{equation*}
		\c{S}_{m,n}=\frac{\c{b}_m}{\c{a}_n}e^{-\j\beta(l_m+l_n)}
	\end{equation*}
Serienimpedanz, Paralleladmittanz und Parallelimpedanz
	\begin{align*}
		\c{\matr{S}}_{^{-}}&=\frac{1}{\c{Z}+2Z_\text{L}}
		\begin{pmatrix}
			\c{Z} & 2Z_\text{L}\\
			2Z_\text{L} & \c{Z}
		\end{pmatrix}
		&
		\c{\matr{S}}_{||}&=\frac{1}{2+\c{Y}Z_\text{L}}
		\begin{pmatrix}
			-\c{Y}Z_\text{L} & 2\\
			2 & -\c{Y}Z_\text{L}
		\end{pmatrix}\\
		& & &=\frac{1}{2\c{Z}+Z_\text{L}}
		\begin{pmatrix}
			-Z_\text{L} & 2\c{Z}\\
			2\c{Z} & -Z_\text{L}
		\end{pmatrix}\\
...
\subsection{Zusammenhang Impedanz-, Admittanz- und Streumatrix}
	\begin{align*}
		\c{\matr{S}}&=\matr{E}-2Z_\text{L}(\c{\matr{Z}}+Z_\text{L}\matr{E})^{-1} & \c{\matr{Z}}&=-Z_\text{L}(\matr{E}+2(\c{\matr{S}}-\matr{E})^{-1})\\
		&=-\matr{E}+2(Z_\text{L}\c{\matr{Y}}+\matr{E})^{-1} & \c{\matr{Y}}&=-\frac{1}{Z_\text{L}}(\matr{E}-2(\c{\matr{S}}+\matr{E})^{-1})
	\end{align*}
%\vfill
\HL
...
\subsection{Ablesen und Antragen der Werte}
	...
	\begin{equation*}
		|\c{\Gamma}|=r=\frac{\c{Z}-Z_\text{L}}{\c{Z}+Z_\text{L}}=\frac{\c{z}-1}{\c{z}+1}=\frac{s-1}{s+1}=\frac{1-m}{1+m}\qquad s=\frac{1}{m}=\frac{1+|r|}{1-|r|}
	\end{equation*}
\HL
\section*{Allgemeines}
	\begin{equation*}
...
\hline
Mikro  & $\si{\micro}$ & $10^{-6}$ & Peta   & P  & $10^{15}$\\
\hline
...
\subsection{Anpassung}
	\begin{equation*}
		P=\frac{1}{4}\frac{U^2}{R_\text{i}}=\frac{1}{4}I^2R_\text{i} \qquad \text{Reflexionsfreiheit und maximaler Wirkungsgrad $\eta=\SI{50}{\percent}$}
	\end{equation*}
\subsection{Koordinatenumwandlung}
	\begin{align*}
		x&=r\sin(\theta)\cos(\varphi) & y&=r\sin(\theta)\sin(\varphi) & z&=r\cos(\theta)\\
		r&=\sqrt{x^2+y^2+z^2} & \theta &=\arctan\left(\frac{\sqrt{x^2+y^2}}{z}\right) & \varphi &=\arctan\left(\frac{y}{x}\right)
	\end{align*}
\subsection{Randbedingungen Helmholtzgleichung}
	\begin{equation*}
		\left.\frac{\partial\c{H}_\text{z}}{\partial x}\right|_{^{x=0}_{x=a}}=0 \qquad \left.\frac{\partial\c{H}_\text{z}}{\partial y}\right|_{^{y=0}_{y=b}}=0 \qquad \left.\c{E}_\text{z}\right|_{^{x=0}_{x=a}}=0 \qquad \left.\c{E}_\text{z}\right|_{^{y=0}_{y=b}}=0
	\end{equation*}
\vfill
\HL\\
\shorthandoff{"}
